/* ─────────────────────────────────────────────────────────
   ...
   Official Simple Icons glyph paths (single, consistent set)
   used by BOTH the footer "Connect With Fortune U Group"
   section and the floating social stack.
────────────────────────────────────────────────────────── */
...
/* Render just the brand glyph (single-color vector). */
export function BrandGlyph({ path, className = "w-6 h-6" }) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="currentColor"
...
/* Circular social button — shared by footer and floating stack.
   variant: "footer" (on dark navy) | "floating" (over page content) */
export function SocialIconButton({ social, variant = "footer", className = "" }) {
  ...
  const variants = {
    footer:
      "bg-white/10 border border-white/15 text-white shadow-md hover:bg-[#D4AF37] hover:border-[#D4AF37] hover:text-[#0A2540] focus-visible:ring-offset-[#0A2540] w-11 h-11",
    floating:
      "bg-white text-[#0A2540] border border-white/70 shadow-[0_4px_14px_rgba(10,37,64,0.18)] hover:bg-[#D4AF37] hover:border-[#D4AF37] hover:text-[#0A2540] focus-visible:ring-offset-white w-11 h-11 sm:w-12 sm:h-12",
  };
  return (
    <a
      href={social.href}
    ...
    >
      <BrandGlyph
        path={social.path}
        className={variant === "floating" ? "w-5 h-5 sm:w-6 sm:h-6" : "w-5 h-5"}
      />
    </a>
  );
}
